"""
Ref:
https://raw.githubusercontent.com/pytorch/vision/master/torchvision/models/
resnet.py
"""
import sys
import threading
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        if groups != 1 or base_width != 64:
            raise ValueError('BasicBlock only supports groups=1 and base_'
                             'width=64')
        if dilation > 1:
...
class Bottleneck(nn.Module):
    ...
    # according to
    # https://ngc.nvidia.com/catalog/model-scripts/nvidia:resnet_
    # 50_v1_5_for_pytorch.
...
class ResNet(nn.Module):
    ...
    def __init__(self, arch, block, layers, num_classes=1000,
        ...
        if isinstance(self.layer4[-1], Bottleneck):
            ...
            raise ValueError("Unsupported class of block {}.".format(
                type(self.layer4[-1])
            ))
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    model = ResNet(arch=arch, block=block, layers=layers, **kwargs)
    if pretrained:
